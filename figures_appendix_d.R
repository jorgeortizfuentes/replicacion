...
savepath <- "save_path"
type <- "full_sample.png"

resultspath <- "results_path" # based on estimates from models_appendix_d_e.R

# these are the figures in the paper

library(checkpoint)
checkpoint("2018-06-15", project = "own_path", checkpointLocation = "own_cp_location")
...
load(file=paste0(resultspath, 'm_8.RData'))
load(file=paste0(resultspath, 'm_9.RData'))
load(file=paste0(resultspath, 'm_10.RData'))
load(file=paste0(resultspath, 'm_11.RData'))
...
eff.m <- effect("region:rideol", m.7[[1]])
...
eff.m <- effect("region:rideol:year_num", m.7[[2]],
...
# ########################################################
# # hystorical legacy at the individual level - figure 3 #
# ########################################################

eff.m <- effect("region:rideol:soc_pop_eleches", m.9[[1]])
...
eff.m <- effect("region:rideol:prtdgcl_num", m.10[[1]],
...
eff.m <- effect("region:gov_dist", m.11[[1]])
eff.m <- as.data.frame(eff.m)
...
eff.m <- effect("region:gov_genlr:gov_dist", m.11[[2]],
